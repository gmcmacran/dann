...
for(i in seq_along(1:nrow(betweenSS))) {
  for(j in seq_along(1:ncol(betweenSS))) {
    ...
    for(k in seq_along(1:nrow(temp))) {
      tempM1 <- groupMeans[k, i] - grandMean[1,i]
      tempM2 <- groupMeans[k, j] - grandMean[1,j]
...
for(i in seq_along(1:nrow(withinSS))) {
  for(j in seq_along(1:ncol(withinSS))) {
    ...
    for(k in seq_along(1:nrow(temp))) {
      ...
      tempM1 <- x0[k, i] - currentGroupMean1
      tempM2 <- x0[k, j] - currentGroupMean2
...
for(i in seq_along(1:nrow(withinSS))) {
  for(j in seq_along(1:ncol(withinSS))) {
    temp <- matrix(-1, nrow = nrow(x0), ncol = 1)
    for(k in seq_along(1:nrow(temp))) {
      tempM1 <- x0[k, i] - grandMean[1,i]
      tempM2 <- x0[k, j] - grandMean[1,j]
...
isSymmetric(totalSS)
all(diag(totalSS)>0)
all(diag(totalSS)^.5>9.8 & diag(totalSS)^.5<10.2)
